...
if (isProd) {
  ...
  server = https.createServer({
    key: fs.readFileSync(`/etc/letsencrypt/live/${config.get('settings:websiteDomain')}/privkey.pem`),
    cert: fs.readFileSync(`/etc/letsencrypt/live/${config.get('settings:websiteDomain')}/fullchain.pem`),
  }, app);
} else {
